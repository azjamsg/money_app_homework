...
// <li class=""></li>
  deposit_item.classList.add('plus'); // there's a style called plus on css
  deposit_item.innerHTML = `
  ${transaction.customername}-${transaction.bank}  <span> $ ${Math.abs(
    transaction.deposit  
  )}</span> 
  `; //whole thing is a string. `quote to make it as such. using template literal
...
function updateValues() {
  const deposits = TransactionData.map(transaction => transaction.deposit);
                  // dep1 = TransactionData.forEach(function(transaction) {return transaction.deposit})
                  // but the above wont map into a new array. prob need another step
                  //alternatively dArray = TransactionData.map(function(transaction) {return transaction.deposit})
                  // alternatively dArray = TransactionData.map(transaction => transaction.deposit)
                  //
...
function filterTransaction(e) {
  ...
  list.innerHTML = '';
  reco.innerHTML = '';
  TransactionData = TransactionDataAll.filter(tran => tran.customername.toLowerCase() == custname.value.toLowerCase());  //custname entered by the user on interface
  TransactionData.forEach(addTransactionDOM);
  updateValues(); 
}

init();
form.addEventListener('submit', filterTransaction); //if 2 buttons, need to put in the button name
